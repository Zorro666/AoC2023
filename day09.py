...
def sequence_next(sequence):
    ...
    while nextSum != 0:
        ...
        for i in range(len(active)-1):
            ...
            next.append(diff)
            nextSum += abs(diff)
        reductions.append(next)
        active = next

    for i in range(len(reductions)-2, -1, -1):
      reductions[i].append(reductions[i][-1] + reductions[i+1][-1])
    return reductions[0][-1]

def sequence_prev(sequence):
    reductions = []
    active = sequence
    reductions.append(sequence)
    nextSum = 1
    ...
    for i in range(len(reductions)-2, -1, -1):
      new_value = reductions[i][0] - reductions[i+1][0]
      reductions[i].insert(0, new_value)
...
def solvePart1(lines):
  ...
  return sum(sequence_next(sequence) for sequence in sequences)
...
def solvePart2(lines):
  ...
  return sum(sequence_prev(sequence) for sequence in sequences)

def main():
    aoc.run_day(9, solvePart1, 1772145754, solvePart2, 867)
