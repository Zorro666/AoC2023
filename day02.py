...
def solvePart1(lines):
    ...
    for l in lines:
        ...
        # Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
        toks = l.strip().split(":")
        game = toks[0].strip().split()[1].strip()
        # 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
        rounds = toks[1].strip().split(";")
        valid = True
        for round in rounds:
            # 3 blue, 4 red
            ...
            for cube in cubes:
                cubeToks = cube.split()
                count = cubeToks[0].strip()
                colour = cubeToks[1].strip()
                counts[colour] += int(count)
            if counts["red"] > maxRed or counts["green"] > maxGreen or counts["blue"] > maxBlue:
                valid = False
                break
            if valid == False:
                break
...
def main():
    aoc.run_day(2, solvePart1, 2879, solvePart2, 65122)
