...
def find_vertical_reflect(grid, w, h, maxMisses):
    for x0 in range(w-1):
        misses = 0
        for x in range(min(x0+1,w - x0 - 1)):
            if x0 - x < 0 or x0 + x + 1 >= w:
                print(f"Out of range x0:{x0} x:{x} w:{w}")
                return (0, maxMisses)
            for y in range(h):
                if grid[y][x0 - x] != grid[y][x0 + x + 1]:
                    misses += 1
                if misses > maxMisses:
                    break
            if misses > maxMisses:
...
def pattern_score(lines, part2):
    ...
    for l in lines:
        ...
        if new_w > 0:
            if w == 0:
                w = new_w
            if w != new_w:
                print("Error: pattern is not rectangular", l, w, new_w)
                return 0
            grid.append(l)
    ...
    for x in range(w):
        ...
        for y in range(h):
            l.append(grid[y][x])
        grid_transpose.append(l)

    maxMisses = 0
    (scoreV, missesV) = find_vertical_reflect(grid, w, h, maxMisses)
    (scoreH, missesH) = find_vertical_reflect(grid_transpose, h, w, maxMisses)

    if part2:
        maxMisses = 1
        (scoreV2, missesV) = find_vertical_reflect(grid, w, h, maxMisses)
        if scoreV2 != scoreV and missesV == maxMisses:
            scoreV = scoreV2
        (scoreH2, missesH) = find_vertical_reflect(grid_transpose, h, w, maxMisses)
        if scoreH2 != scoreH and missesH == maxMisses:
            scoreH = scoreH2
...
def solvePart1(lines):
    ...
    for l in lines:
        l = l.strip()
        if len(l) == 0 and len(pattern) != 0:
            result += pattern_score(pattern, False)
            pattern = []
        if len(l):
            pattern.append(l)
    result += pattern_score(pattern, False)
...
def solvePart2(lines):
    ...
    for l in lines:
        l = l.strip()
        if len(l) == 0 and len(pattern) != 0:
            result += pattern_score(pattern, True)
            pattern = []
        if len(l):
            pattern.append(l)
    result += pattern_score(pattern, True)
...
def main():
    aoc.run_day(13, solvePart1, 29165, solvePart2, 456)
