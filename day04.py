...
def solvePart1(lines):
    ...
    for l in lines:
        ...
        for n in numbers:
            if n in winners:
                total += 1
        cardsWinCount.append(total)
    result = 0
    for winCount in cardsWinCount:
        if winCount > 0:
            result += 2 ** (winCount - 1)
    return result
...
def solvePart2(lines):
    ...
    cardCounts = [1] * len(cardsWinCount)
    for c in range(0, len(cardsWinCount)):
        winCount = cardsWinCount[c]
        for j in range(c+1, c+1+winCount):
            cardCounts[j] += cardCounts[c]
...
def main():
    aoc.run_day(4, solvePart1, 23235, solvePart2, 5920640)
