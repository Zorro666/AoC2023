...
"Oh, all of Gear Island is currently offline! Nothing is being manufactured at the moment, not until we get more lava to heat our forges. 
And our springs. 
...
However, the engineer that produced the condition records also duplicated some of this information in a different format! After the list of springs for a given row, the size of each contiguous group of damaged springs is listed in the order those groups appear in the row. 
...
(Neither ?? could be both broken springs or they would form a single contiguous group of two; if that were true, the numbers afterward would have been 2,3 instead.) Since each ?? can either be #. or .#, there are four possible arrangements of springs.

The last line is actually consistent with ten different arrangements! Because the first number is 3, the first and second ? must both be . 
...
# ???.### 1,1,3
    ...
    for t in toks[1].split(','):
        counts.append(int(t))
    return line, tuple(counts)

@cache
def validMatches(line, counts):
    ...
    for i in range(len(line)):
        ...
        if c == '.' or c == '?':
            if match == target:
                return validMatches(line[i+1:], counts[1:])
        if c == '.':
            if match == 0:
                continue
            if match != target:
        ...
        if c == '?':
            matches = 0
            if match < target:
                newLine = line[i-match:i] + '#' + line[i+1:]
                matches += validMatches(newLine, counts)
            newLine = line[i-match:i] + '.' + line[i+1:]
            matches += validMatches(newLine, counts)
            return matches
        if match > target:
...
def countMatches(line):
    l, counts = parseLine(line, trim=True)
    return validMatches(l, counts)

def countMatchesUnFolded(line):
# .# 1 -> .#?.#?.#?.#?.# 1,1,1,1,1
    foldedLine, foldedCounts = parseLine(line, trim=False)
    l = foldedLine + "?"
    l += foldedLine + "?"
    l += foldedLine + "?"
    l += foldedLine + "?"
    l += foldedLine
    counts = foldedCounts * 5
    return validMatches(l, counts)
...
def solvePart2(lines):
    ...
    for l in lines:
        if len(l) == 0:
            continue
        result += countMatchesUnFolded(l)
    return result

def main():
    aoc.run_day(12, solvePart1, 7939, solvePart2, 850504257483930)
