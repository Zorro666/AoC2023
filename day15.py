...
def solvePart2(lines):
    boxes = []
    boxes.extend([[] for i in range(256)])
    if len(boxes) != 256:
        raise Exception("Expected 256 boxes")
    for l in lines:
        l = l.strip()
        if len(l) == 0:
            continue
    ...
    for i in range(len(boxes)):
        ...
        for j in range(len(lenses)):
            ...
            if lens[1] == 0:
                continue
            if len(lens[0]) == 0:
                raise Exception("Expected non-empty label")
            result += (i + 1) * (j + 1) * lens[1]
    return result

def main():
    aoc.run_day(15, solvePart1, 494980, solvePart2, 247933)
